...
router.get('/', (req, res, next) => {
    ...
    console.log(`moira champion analyzing... rank: ${rank}`);
    let min, max;
    {
        if(rank < 1500){
            min = 500;
    ...
    const aggregation = User.aggregate([
        ...
        },
        {
            $group:
            {
                ...
                '게임당_융화로 처치': {
                    $avg: { $divide: ['$rankplay.record.모이라.영웅별.융화로 처치', '$rankplay.record.모이라.게임.치른 게임' ] }
                },
                '게임당_융화로 처치_최소': {
                    $min: {
                        $avg: {
                            $divide: ['$rankplay.record.모이라.영웅별.융화로 처치', '$rankplay.record.모이라.게임.치른 게임' ]
                        }
                    }
                },
                '게임당_융화로 처치_최대': {
                ...
                '게임당_융화로 치유': {
                    $avg: { $divide: ['$rankplay.record.모이라.영웅별.융화로 치유', '$rankplay.record.모이라.게임.치른 게임' ] }
                },
                '게임당_융화로 치유_최소': {
                    $min: {
                        $avg: {
                            $divide: ['$rankplay.record.모이라.영웅별.융화로 치유', '$rankplay.record.모이라.게임.치른 게임' ]
                        }
                    }
                },
                '게임당_융화로 치유_최대': {
                ...
                '게임당_자가 치유': {
                    $avg: { $divide: ['$rankplay.record.모이라.영웅별.자가 치유', '$rankplay.record.모이라.게임.치른 게임' ] }
                },
                '게임당_자가 치유_최소': {
                    $min: {
                        $avg: {
                            $divide: ['$rankplay.record.모이라.영웅별.자가 치유', '$rankplay.record.모이라.게임.치른 게임' ]
                        }
                    }
                },
                '게임당_자가 치유_최대':{
