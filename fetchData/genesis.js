...
mongoose.connect(
  "mongodb+srv://eunsu:dmstn~0909@overfetch-beta-release-noxxr.mongodb.net/test?retryWrites=true",
...
const isUserDuplicate = async (name, tag) => {
  const result = await User.findOne({ name: name, tag: tag })
    .then(user => {
      if (!user) {
        return new Promise(resolve => {
          resolve(false);
      ...
      if (data.error) {
        ...
        const promises = await data.map(async el => {
          ...
          if (el.isPublic == true) {
            ...
            // 먼저 중복 여부 검사
            const isDuplicate = await isUserDuplicate(_name, tag);
            if (isDuplicate == true) {
            ...
            if (typeof userInfo == "number") {
              ...
              switch (userInfo) {
                case FetchManager.ERROR_RESULT.PROFILE_NOT_FOUND: {
                  newError.message =
                    "프로필을 찾을 수 없습니다. 닉네임과 배틀태그를 확인해 주세요";
                  break;
                }
        ...
        console.log(
          `이름이 <${name}>인 유저들의 데이터 수집이 완료되었습니다...`
        );
        return new Promise(resolve => {
          resolve(name);
        });
